...
def find_citations(text, df, chapters):
    ...
    for line in text.split('\n'):
        ...
        for _, row in df.iterrows():
            pattern = re.escape(row['Author']) + r'.*\b' + str(row['Year']) + r'\b'
...
def process_reports(excel_path, text_directory):
    ...
    with pd.ExcelWriter(excel_path[:-5] + '_updated.xlsx') as writer:
        for sheet_name in xls.sheet_names:
            ...
            print(f"Processing report: {sheet_name}")
            for file in matching_files:
                print(f"Processing file: {file}")
                with open(os.path.join(text_directory, file), 'r', encoding='utf-8') as f:
                    text = f.read()
                    df = find_citations(text, df, chapters)
            df.to_excel(writer, sheet_name=sheet_name)  # Save updated DataFrame to sheet

excel_path = './data/cci/matched_references.xlsx'
