...
def plot_project_counts(project_counts, tag):
    ...
    plt.savefig(f'{output_dir}/project_counts_{tag}.pdf')
    print(f'Plot saved as {output_dir}/project_counts_{tag}.pdf')

# Example usage
for tag in ['wg1', 'wg2', 'wg3']:
    file_path = f'./results/matched_references_{tag}.bib'  # Replace with the path to your BibTeX file
    bib_database = parse_bib_file(file_path)
    project_counts = count_projects(bib_database)
    plot_project_counts(project_counts, tag)
